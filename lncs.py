...
for i in range(0,983):
	...
	if cnt >= maxYnodes:
		...
		s_nodes = [n for n,attrdict in G.node.items() if attrdict['start'] == 1]
		e_nodes = [n for n,attrdict in G.node.items() if attrdict['end'] == 1]
		for s in s_nodes:
			for e in e_nodes:
	...
	if flag == '1': #if fact
		...
		while len(tokens) > 1:
			...
			if tfMap.has_key(tokens[0]):
				...
				tfMap[tokens[0]] = 1
			if fl != 0:
				G.add_node(tokens[0],start=0,end=0)
			if fl == 0:
				G.add_node(tokens[0],start=1,end=0)
				fl = 1
